...
require "behaviours/nodes/iterate_and_doaction"
...
local FARM_TILE_RADIUS = 2.7
local FIND_ENTITIES_TOLERANCE_RADIUS = 0.21
...
local Fillers = {
    ...
    UnwantedEntities = function(inst, iterator)
        ...
        for _, tile in orderedPairs(tiles) do
            local targets = TheSim:FindEntities(tile.x, tile.y, tile.z, FARM_TILE_RADIUS, nil, SOILMUST, FARM_ENTITIES_TAGS)
    ...
    SpotsToTill = function(inst, iterator)
        ...
        for _, tile in orderedPairs(tiles) do
            for _, slot in pairs(SortSlots(tile.slots)) do
                ...
                local localsoils = TheSim:FindEntities(pos.x, pos.y, pos.z, FIND_ENTITIES_TOLERANCE_RADIUS, SOILMUST, SOILMUSTNOT)
    ...
    SoilsToPlant = function(inst, iterator)
        ...
        for _, tile in orderedPairs(tiles) do
            for _, slot in pairs(SortSlots(tile.slots)) do
                local localsoils = TheSim:FindEntities(slot.x, slot.y, slot.z, FIND_ENTITIES_TOLERANCE_RADIUS, SOILMUST, SOILMUSTNOT)
                local localplants = TheSim:FindEntities(slot.x, slot.y, slot.z, FIND_ENTITIES_TOLERANCE_RADIUS, nil, nil, FARM_PLANT_TAGS)
    ...
    PlantsToHarvest = function(inst, iterator)
        ...
        for _, tile in orderedPairs(tiles) do
            for _, slot in pairs(SortSlots(tile.slots)) do
                local pos = Vector3(slot.x, slot.y, slot.z)
                local localplants = TheSim:FindEntities(pos.x, pos.y, pos.z, FIND_ENTITIES_TOLERANCE_RADIUS, nil, nil, FARM_PLANT_TAGS)
...
function MermBrain:OnStart()
    ...
    local CollectToolFromShed = IfNode(function() return NeedsToolAndFoundTool(self.inst) end, "needs a tool",
        PriorityNode({
            Leash(self.inst, GetClosestToolShedPosition, 20.1, 20, true),
    ...
    local IteratorNodes = {
        ...
        HammerOversized = IterateAndDoActionNode(self.inst, {
            name = "HAMMER_OVERSIZED_PLANTS",
            starter = Fillers.PlantsToHammer,
            action = Actions.Hammer,
            run = true
        }),
        DigUnwanted = IterateAndDoActionNode(self.inst, {
