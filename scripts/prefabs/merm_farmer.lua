...
local merm_farmer_brain = require "brains/merm_farmerbrain"
local SLIGHTDELAY = 1
...
local function ShouldAcceptItem(inst, item, giver)
  ...
  local all = (item.components.equippable.equipslot == EQUIPSLOTS.HEAD or item.components.equippable.equipslot == EQUIPSLOTS.HANDS or item.components.equippable.equipslot == EQUIPSLOTS.BODY)
  return (giver:HasTag("merm") and not (inst:HasTag("mermguard") and giver:HasTag("mermdisguise"))) and
      ((item.components.equippable ~= nil and (all)) or
...
local function OnGetItemFromPlayer(inst, giver, item)
  ...
  -- I also wear hats
  local all = (item.components.equippable.equipslot == EQUIPSLOTS.HEAD or item.components.equippable.equipslot == EQUIPSLOTS.HANDS or item.components.equippable.equipslot == EQUIPSLOTS.BODY)
  if item.components.equippable ~= nil and (all) then
    ...
    if equipslot == EQUIPSLOTS.HEAD then
...
local function common_master(inst)
  ...
  inst.farm_layout_blueprint = {
    plant_list = {}
  }
  inst:SetStateGraph("SGmerm_farmer")
  ...
  local function EquipItem(inst, item)
    local prefab = SpawnPrefab(item)
    inst.components.inventory:Equip(prefab)
  end
  EquipItem(inst, "farm_hoe")
  EquipItem(inst, "plantregistryhat")
end

return MakeMerm("merm_farmer", assets, prefabs, common_common, common_master)
