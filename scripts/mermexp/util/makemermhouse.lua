...
local function onhit(inst, worker)
  if not inst:HasTag("burnt") then
    if inst.components.childspawner ~= nil then
      inst:ReleaseAllChildren(worker)
    end
...
local function HasKingAnywhere()
  return TheWorld.components.mermkingmanager:HasKingAnywhere()
end
...
local function ReleaseAllChildren(inst, worker)
  if inst.needskingtorelease and not HasKingAnywhere() then return end
...
local function mermhouse_master_base(inst)
  ...
  inst.components.childspawner:SetOnTakeOwnershipFn(function(inst, child)
    child:ListenForEvent("onremove", function() child:QuitJob() end, inst)
  end)
end

local function SetNeedsKingToSpawn(inst, enable) inst.needskingtospawn = enable end
local function SetNeedsKingToRelease(inst, enable) inst.needskingtorelease = enable end

local function SetSpawnOnKingCreated(inst, enable)
  if enable then
    if inst.spawnchildren then
      inst:RemoveEventCallback("onmermkingcreated", inst.spawnchildren, TheWorld)
    end

    inst.spawnchildren = function()
      local childspawner = inst.components.childspawner
    ...
    inst:ListenForEvent("onmermkingcreated", inst.spawnchildren, TheWorld)
  elseif inst.spawnchildren then
    inst:RemoveEventCallback("onmermkingcreated", inst.spawnchildren, TheWorld)
  end
end


local function MakeMermHouse(name, assets, prefabs, common_postinit, master_postinit)
  local function fn()
    local inst = CreateEntity()
    ...
    if not TheNet:IsDedicated() and not inst.components.deployhelper then
    ...
    inst.SetNeedsKingToSpawn = SetNeedsKingToSpawn
    inst.SetNeedsKingToRelease = SetNeedsKingToRelease
    inst.SetSpawnOnKingCreated = SetSpawnOnKingCreated

    inst.StartSpawning = StartSpawning
    inst.StopSpawning = StopSpawning
    inst.ReleaseAllChildren = ReleaseAllChildren

    mermhouse_master_base(inst)
...
---@param assets table
---@param prefabs table
---@param common_postinit fun(inst)
---@param master_postinit fun(inst)
---@param placer_cfg? {bank: string, build: string, anim: string, onground: any, snap: any, metersnap: any, scale: any, fixedcameraoffset: any, facing: any, postinit_fn: function, offset:any, onfailedplacement: fun(player, placer)} | true
---@return any, any
return function(name, assets, prefabs, common_postinit, master_postinit, placer_cfg)
  local placer = type(placer_cfg) == "table" and
      MakePlacer(name .. "_placer", placer_cfg.bank or name, placer_cfg.build or name,
        placer_cfg.anim or "idle",
        placer_cfg.onground,
        placer_cfg.snap,
        placer_cfg.metersnap,
        placer_cfg.scale,
        placer_cfg.fixedcameraoffset,
        placer_cfg.facing,
        placer_cfg.postinit_fn,
        placer_cfg.offset,
        placer_cfg.onfailedplacement or invalid_placement_fn
      )
      or
      MakePlacer(name .. "_placer", name, name, "idle", nil, nil, nil, nil, nil, nil, nil, nil, invalid_placement_fn)


  return MakeMermHouse(name, assets, prefabs, common_postinit, master_postinit), placer
end
