...
local function OnAttackedByDecidRoot(inst, attacker)
  ...
  local combat_helpers = TheSim:FindEntities(x, y, z, SpringCombatMod(share_target_dist) * .5, DECIDROOTTARGET_MUST_TAGS,
...
local function GetOtherMerms(inst, radius, maxcount, giver)
  ...
  local merms = TheSim:FindEntities(x, y, z, radius, MERM_TAGS,
    giver and giver:HasTag("playermerm") and MERM_IGNORE_TAGS or MERM_IGNORE_TAGS_NOGUARDS)
...
local function common_master(inst)
  ...
  function inst:GetHome()
    if inst.components.homeseeker == nil then return nil end
    return inst.components.homeseeker:GetHome()
  end

  local function StringMatches(a, b, exact)
    if exact then return a == b end
    return a:find(b)
  end

  function inst:FetchItemInSelf(item_name, options)
    ...
    local containers = {
      ...
      (options.include_home and inst:GetHome() ~= nil) and inst:GetHome().components.container or nil
    }

    for _, container in ipairs(containers) do
      local item = container:FindItem(function(item) return StringMatches(item.prefab, item_name, options.exact) end)
      if item ~= nil then return options.remove and container:RemoveItem(item) or item end
    end
  end

  function inst:Equip(item_name, equipslot, options)
    options = options ~= nil and options or {}

    if inst.components.inventory ~= nil then
      local item = inst.components.inventory:GetEquippedItem(equipslot)
      if item ~= nil and item.prefab:find(item_name) then return item end

      local item = inst:FetchItemInSelf(item_name, {
        include_home = options.try_equip_from_home,
        remove = true,
        equipslot = equipslot
      })

      if item ~= nil then inst.components.inventory:Equip(item) end

      return item
    end
  end

  function inst:Unequip(equipslot, options)
    options = options ~= nil and options or {}

    local equipitem = inst.components.inventory:GetEquippedItem(equipslot)
  ...
  function inst:QuitJob()
    self.components.inventory:DropEverything()
    if self.components.container then
      self.components.container:DropEverything()
    end

    local merm = SpawnPrefab("merm")
    merm.Transform:SetPosition(self.Transform:GetWorldPosition())
    self:Remove()
  end
end
