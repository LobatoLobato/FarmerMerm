...
local MapWidget = require("widgets/fmapwidget")
local Widget = require "widgets/widget"
local MapControls = require "widgets/fmapcontrols"
...
local MapScreen = Class(Screen, function(self, owner)
    ...
    self.bottomright_root.inst:ListenForEvent("refreshhudsize",
        function(hud, scale) self.bottomright_root:SetScale(scale) end, owner.HUD.inst)
        ...
        if charlieresidue and charlieresidue:IsValid() then
            ...
            if context > CHARLIERESIDUE_MAP_ACTIONS.NONE then
                if context == CHARLIERESIDUE_MAP_ACTIONS.WORMHOLE then
                    ...
                    local zoomscale_clamped = math.clamp(zoomscale, minzoomscale or zoomscale, maxzoomscale or zoomscale) *
                        overallzoomscaler
...
function MapScreen:UpdateStaticDecorations()
    ...
    for _, decorationdata in pairs(staticdecorations) do
        ...
        if ent:IsValid() then
            local ex, ey, ez = ent.Transform:GetWorldPosition()
            decoration:Show()
            local zoomscale_clamped = math.clamp(zoomscale, decorationdata.minzoomscale or zoomscale,
                decorationdata.maxzoomscale or zoomscale) * (decorationdata.overallzoomscaler or 1)
            local x, y = self.minimap:WorldPosToMapPos(ex, ez, 0)
            decoration:SetPosition(x * w, y * h)
            decoration:SetScale(zoomscale_clamped, zoomscale_clamped, 1)
...
function MapScreen:ProcessRMBDecorations_JUMPIN_MAP(rmb, fresh)
    ...
    if charlieresidue and charlieresidue:IsValid() then
        ...
        if context > CHARLIERESIDUE_MAP_ACTIONS.NONE then
            if context == CHARLIERESIDUE_MAP_ACTIONS.WORMHOLE then
                local ents = TheSim:FindEntities(rmb_pos.x, rmb_pos.y, rmb_pos.z,
                    TUNING.SKILLS.WINONA.WORMHOLE_DETECTION_RADIUS, MAP_SELECT_WORMHOLE_MUST)
                for _, ent in ipairs(ents) do
                    local ex, ey, ez = ent.Transform:GetWorldPosition()
...
function MapScreen:OnUpdate(dt)
    ...
    if inoutdir ~= 0 then
        ...
        self.zoom_target = math.clamp(self.zoom_target + self.zoomsensitivity * inoutdir * exponential_factor,
            ZOOM_CLAMP_MIN, ZOOM_CLAMP_MAX)
...
function MapScreen:GetHelpText()
    ...
    table.insert(t,
        TheInput:GetLocalizedControl(controller_id, CONTROL_ROTATE_LEFT) .. " " .. STRINGS.UI.HELP.ROTATE_LEFT)
    table.insert(t,
        TheInput:GetLocalizedControl(controller_id, CONTROL_ROTATE_RIGHT) .. " " .. STRINGS.UI.HELP.ROTATE_RIGHT)
    ...
    if playercontroller and playercontroller.RMBaction then
        table.insert(t,
            ...
            " " .. playercontroller.RMBaction:GetActionString())
    end
